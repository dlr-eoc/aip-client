...
class AIPDownloader(Downloader):
    ...
    def __init__(self, api, *, node_filter=None, verify_checksum=True, fail_fast=False, n_concurrent_dl=None,
        ...
        from aip_client.client import AIPClient
        self.api: AIPClient = api

        self.verify_checksum = False  # Calculates checksum after the download
        self.verify_md5_dl = False  # Calculates the checksum during the download and compares it after completion without re-reading the file.
    ...
    def download_all(self, products: list = None, directory=".", batch_id: str = None):
        ...
        statuses, online_prods, offline_prods, product_infos, exceptions = self._init_statuses(products,
                                                                                               batch_id=batch_id,
                                                                                               directory=directory)
    ...
    def _init_statuses(
        ...
        for pid in self._tqdm(
            ...
            if batch_id is None:
                ...
                product_infos[pid["Id"]]["url"] = "{}odata/v1/BatchOrder({})/Products({})/$value".format(
                    self.api.api_url, batch_id, pid["Id"])
                statuses[pid["Id"]] = DownloadStatus.ONLINE
                online_prods.add(pid["Id"])
    ...
    def download(self, id, product_info=None, trigger_offline=True, wait_for_reload=True, directory=".", *,
                 stop_event=None):
        if self.node_filter:
        ...
        if product_info["batch_product"]:
            ...
            if wait_for_reload:
                ...
                if not self.wait_for_aip_offline_retrieval(id):
    ...
    def wait_for_aip_offline_retrieval(self, id):
    ...
    def _download_common(self, product_info: Dict[str, Any], path: Path, stop_event):
        ...
        if not skip_download:
            ...
            product_info["downloaded_bytes"], md5 = self._download_aip(
                product_info["url"],
                temp_path,
        ...
        if self.verify_md5_dl is True:
            if not product_info["Checksum"][0]["Value"] == product_info["md5"]:
                temp_path.unlink()
                raise InvalidChecksumError("File corrupt: checksums do not match")
    ...
    def trigger_offline_retrieval(self, uuid):
        ...
        self.api._check_aip_response(r, test_json=False)
    ...
    def _download_aip(self, url, path, file_size, title, stop_event):
        headers = {}
        ...
        with self._tqdm(
            ...
            return downloaded_bytes, md5_digest
