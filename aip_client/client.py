...
from aip_client.models import *
from aip_client.downloader import AIPDownloader


class AIPClient(SentinelAPI):
    """Class to connect to Copernicus Space Component Ground Segment
    Archive Interface delivery Point, search and download products.
    ...
    logger = logging.getLogger("sentinelsat.AIPClient")
    ...
    def __init__(
        ...
        self.downloader = AIPDownloader(self)
    ...
    def download_all(
        ...
        statuses, exceptions, product_infos = downloader.download_all(
            products=products, directory=directory_path, batch_id=batch_id
        )
    ...
    def query(self, query, additional_options=None, skip_odata_output: bool = True):
        """Execute a query to the AIP API with query string.
        ...
        skip_odata_output: bool, optional
            The AIP API returns OData formatted JSON, this option is a way to get only
        ...
        self._check_aip_response(response, query_string=query)
    ...
    def inspect_product(self, id):
        ...
        self._check_aip_response(response)
        return response.json()
    ...
    def bulk_delete(self, bulk_id: str, delete_batches: bool = False):
        ...
        self._check_aip_response(response, test_json=False)
        return True
    ...
    def bulk_auto_download(
        ...
        * Delete Bulk
        ...
        See Also
        --------
        :meth:`AIPClient.batches_auto_download()`
        """
    ...
    def batches_auto_download(self, batches: list, directory=".", checksum=False):
        ...
        * Download Batch
        ...
        See Also
        --------
        :meth:`AIPClient.batch_order_completed()`
        :meth:`AIPClient.batch_order_trigger()`
        :meth:`AIPClient.batch_order_wait_completed()`
        """
        downloader = copy(self.downloader)
        downloader.verify_checksum = checksum
    ...
    def batch_order_ls_products(self, batch_id: str) -> list:
        ...
        See Also
        --------
        :meth:`AIPClient.batch_order_ls_product_ids()`
        """
    ...
    def batch_order_ls_product_ids(self, batch_id: str):
        ...
        See Also
        --------
        :meth:`AIPClient.batch_order_ls_products()`
        """
        batch_products_uri = "{}odata/v1/BatchOrder({})/Products".format(self.api_url, batch_id)
        with self.dl_limit_semaphore:
            response = self.session.get(batch_products_uri)
    ...
    def batch_order_trigger(self, batch_id: str, priority=10):
        ...
        self._check_aip_response(response, expected_status_code=204, test_json=False)
        self.logger.info("Successful triggered batch with id {}".format(batch_id))

    @staticmethod
    def _check_aip_response(
...
def _parse_aip_response(product):
    output = {
        "id": product["Id"],
        "title": product["Name"],
